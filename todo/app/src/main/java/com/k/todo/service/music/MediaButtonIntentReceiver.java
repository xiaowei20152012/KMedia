...
public class MediaButtonIntentReceiver extends BroadcastReceiver {
        ...
        public void handleMessage(final Message msg) {
            switch (msg.what) {
                case MSG_HEADSET_DOUBLE_CLICK_TIMEOUT:
                    ...
                    switch (clickCount) {
                        case 1:
                            command = MusicPlayService.ACTION_TOGGLE_PAUSE;
                            break;
                        case 2:
                            command = MusicPlayService.ACTION_SKIP;
                            break;
                        case 3:
                            command = MusicPlayService.ACTION_REWIND;
                            break;
                        default:
                            command = null;
                            break;
        ...
        if (Intent.ACTION_MEDIA_BUTTON.equals(intentAction)) {
            ...
            switch (keycode) {
                case KeyEvent.KEYCODE_MEDIA_STOP:
                    command = MusicPlayService.ACTION_STOP;
                    break;
                ...
                case KeyEvent.KEYCODE_MEDIA_PAUSE:
                    command = MusicPlayService.ACTION_PAUSE;
                    break;
                case KeyEvent.KEYCODE_MEDIA_PLAY:
                    command = MusicPlayService.ACTION_PLAY;
                    break;
            }
            if (command != null) {
    ...
    private static void startService(Context context, String command) {
        final Intent intent = new Intent(context, MusicPlayService.class);
